...
def combine_images(root_dir):
    ...
    for subdir, _, files in os.walk(root_dir):
        ...
        avif_images = sorted([f for f in files if '-original-' in f and 'avif' in f])
        webp_images = sorted([f for f in files if '-original-' in f and 'webp' in f])
        ...
        for img_list, compression in [(avif_images, 'avif'), (webp_images, 'webp')]:
            if img_list:
                ...
                combined_image = Image.new('RGB', (720, combined_height), color='white')
                ...
                for img, height in zip(img_list, resized_heights):
                    ...
                    quality = img.split('-original-')[1].split('-')[0]
                ...
                base_name = img_list[0].split('-original-')[0]
                combined_image.save(os.path.join(subdir, f'{base_name}-{compression}-Combined.jpeg'))
